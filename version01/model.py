...
class ZumaModel:
    def __init__(self, json_info: JsonInfo):
        self._lives: int = json_info['lives']
        self._grid_size: list[int] = json_info['grid_size']
        self._exp: int = 0
        self._gamestate: GameState = GameState.ROUND_SETUP
        self._colors_str: list[str] = json_info['colors']
        self._rounds: dict[str, RoundInfo] = json_info['rounds']
        self._round_manager: RoundManager = RoundManager(self._grid_size, self._rounds["1"], json_info["colors"])
    ...
    def shooter(self) -> Shooter:
        return self._round_manager.shooter
    @property
    def next_bullet_color(self) -> Color:
        return self._round_manager.next_bullet_color
    @property
    def exp(self) -> int:
        return self._exp
    @property
