...
class RoundManager:
    def __init__(self, grid_size: list[int], round_info: RoundInfo, colors_str: list[str]):
        self._grid_size: list[int] = grid_size
        self._rng: Random = Random(67)
        self._curr_round: int = 0
        self._colors: list[Color] = [Color(color) for color in colors_str]
    ...
    def prepare_round(self, round_info: RoundInfo, exp: int, colors_str: list[str]):
        # called only when curr_round is not 1 - nope
